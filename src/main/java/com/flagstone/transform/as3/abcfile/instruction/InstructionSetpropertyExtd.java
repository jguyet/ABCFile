...
public class InstructionSetpropertyExtd
extends InstructionSetproperty {
    private MultinameInfo multiname_info;
    protected CpoolInfo cpool;

    public InstructionSetpropertyExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.cpool = cpool;
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionSetpropertyExtd(MultinameInfo multiname_info, int position) {
