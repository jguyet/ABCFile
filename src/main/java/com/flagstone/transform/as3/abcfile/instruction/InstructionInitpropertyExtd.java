...
public class InstructionInitpropertyExtd
extends InstructionInitproperty {
    private MultinameInfo multiname_info;
    protected CpoolInfo cpool;

    public InstructionInitpropertyExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.cpool = cpool;
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionInitpropertyExtd(MultinameInfo multiname_info, int position) {
