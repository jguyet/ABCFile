...
public class InstructionGetlexExtd
extends InstructionGetlex {
    private MultinameInfo multiname_info;
    protected CpoolInfo cpool;

    public InstructionGetlexExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.cpool = cpool;
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionGetlexExtd(MultinameInfo multiname_info, int position) {
