...
public class InstructionCallsupervoidExtd
extends InstructionCallsupervoid {
    private MultinameInfo multiname_info;
    private CpoolInfo cpool;

    public InstructionCallsupervoidExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionCallsupervoidExtd(MultinameInfo multiname_info, int arg_count, int position) {
