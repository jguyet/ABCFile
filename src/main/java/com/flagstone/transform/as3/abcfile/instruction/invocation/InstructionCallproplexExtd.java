...
public class InstructionCallproplexExtd
extends InstructionCallproplex {
    private MultinameInfo multiname_info;
    protected CpoolInfo cpool;

    public InstructionCallproplexExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.cpool = cpool;
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionCallproplexExtd(MultinameInfo multiname_info, int arg_count, int position) {
