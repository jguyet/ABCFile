...
public class InstructionFindpropertyExtd
extends InstructionFindproperty {
    private MultinameInfo multiname_info;
    protected CpoolInfo cpool;

    public InstructionFindpropertyExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.cpool = cpool;
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionFindpropertyExtd(MultinameInfo multiname_info, int position) {
