...
public class InstructionAstypeExtd
extends InstructionAstype {
    private MultinameInfo multiname_info;
    protected CpoolInfo cpool;

    public InstructionAstypeExtd(ByteBufferFlash date, int position, CpoolInfo cpool) {
        super(date, position);
        this.cpool = cpool;
        this.multiname_info = ((MultinameInfo) cpool.getMultinames().get(this.index));
    }

    public InstructionAstypeExtd(MultinameInfo multiname_info, int position) {
